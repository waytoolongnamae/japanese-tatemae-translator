...
class OpenAIProvider(LLMProvider):
    """
    OpenAI API provider for intent detection and text refinement.
    """
...
**high（高忠実度）**:
- 元の意味とトーンを最大限保持する - 否定は否定として、批判は批判として伝える
- 直接的な表現をできるだけ維持し、不適切にならない程度の最小限の調整のみ
- **婉曲表現や間接表現は使わない** - 特に以下を禁止：
  - 否定的な内容を「興味深い」「魅力的」などの肯定表現で包まない
  - 「検討させていただく」「参考にさせていただく」などの婉曲的拒絶を使わない
  - 「慎重に」「前向きに」などの曖昧表現を避ける
- 具体的な内容を削除したり置き換えたりしない
- 元の否定的・批判的トーンを保持する
- 「礼儀正しいが率直」な表現 - 敬語は使うが意味は直接的
- 建前技法は使わない（クッション言葉も最小限）
- 例:
  - "I'm not interested" → 「興味がございません」「関心がございません」
  - ×「魅力的ですが今回は見送ります」×「興味深いですが検討します」
  - "won't work" → 「うまくいかないと思います」「実現困難です」
  - ×「興味深いアプローチですが」×「検討が必要です」
...
説明・注釈・引用符は一切不要です。自然な日本語として成立する文章を生成してください。"""
        ...
        try:
            # Adjust system message based on fidelity level
            if fidelity == "high":
                system_message = "あなたは礼儀正しい日本語の専門家です。敬語を使いますが、建前表現は使いません。元のメッセージの意味を正確に、率直に伝えます。否定的な内容は否定的に、批判的な内容は批判的に表現します。「興味深い」「魅力的」などの肯定的な社交辞令は一切使いません。"
            else:
                system_message = "あなたは日本の高文脈コミュニケーションの専門家であり、特に京都の伝統的な建前表現に精通しています。表層では極めて丁寧で肯定的な表現を用いながら、深層では本音を巧妙に暗示する技法を習得しています。言語学的な洗練と文化的な深みを備えた、自然で美しい日本語を生成します。元のメッセージの具体的な内容・文脈は必ず保持しつつ、間接的な表現に変換します。"
            ...
            logger.error(f"Error refining text with OpenAI: {e}")
            raise
